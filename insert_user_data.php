<?php
  ...
  include_once 'dbconn.php'; // 외부 파일 가져오기 : require 사용 가능 (여러번사용가능)
  ...
  // 2. 데이터 조회
  // SELECT [field name](* : 전체 조회) FROM [tablename];
  ...
  // $sql_select = "SELECT * FROM user WHERE user_idx = 9";
  // $result = mysqli_query($db_conn, $sql_select);
  // 2-2. 데이터 정렬
  ...
  // $result = mysqli_query($db_conn, $sql_select);
  // 2-3. 데이터 갯수 지정 조회
  // SELECT [field name](* : 전체 조회) FROM [tablename] LIMIT [start index], [number(lenght)]
  // $sql_select = "SELECT * FROM user WHERE user_name = 'asd' ORDER BY user_idx DESC LIMIT 0, 4";
  // $result = mysqli_query($db_conn, $sql_select);
  ...
  $sql_update = "UPDATE user SET user_name = '김철수' WHERE user_idx = 1";
  ...
  $sql_delete = "DELETE FROM user WHERE user_name = 'cayc2'";
